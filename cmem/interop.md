...
**CONTRACT VERSION: 26 — wazmrt LEADS (owner, 2026-08-19).** wazmrt is finishing its hardening stage and **its copy is the latest**; wasmrt mirrors from it and **does not originate version numbers** until wasmrt reaches the same stage. ✅ **THE 2026-09-19 OWNER DECISIONS ARE FOLDED IN AND NUMBERED HERE** — **§2.4a as v11**, **§2.5 as v12**, wasmrt's **§2.3m annex as v13** (the §2.3 Z1 row, and row 3 promoted to ✅ AGREED), 🔒 **the owner's NO-"LOOKS LIKE" clarification as v14 (§2.4a-i)**, and 🔒 **the owner's same-day NARROWING of §2.4a as v15** (after the module path a single-dash token is the guest's). ⚠️ **v14 puts §2.2's `--dir` drive-letter fallback in tension and needs an owner ruling (§5 #10).** 🆕 **v16 adds §2.4b: a flag with a REQUIRED position, written where it cannot apply, is an error** — and wazmrt has **shipped** it. 🆕 **v17–v20 fold in wasmrt's T9e + T9i (2026-09-19):** **v17** Z4, `.wat` pin digests are not portable and the cause is the `name` section · **v18** the pin DB path is DECIDED (§3.3), closing §5 #1 · **v19** §3.7a is CLOSED, verified by running both, closing §5 #5 · **v20** §2.1/§2.2 convergence — wasmrt's column is green and wazmrt's halves are what remain. ⚠️⚠️ **v20 also carries a NEW wazmrt defect this pass found by running: `--dir .:/` — the example wazmrt's own `--help` prints — DOES NOT WORK**, and the cause is the drive-letter heuristic already flagged at §5 #10. 🚨 **v15 exists because §1c happened again while this very pass was running** — both sessions edited their own copies within the same hour; see the v15 change-log row. 📎 **The annexes themselves (§2.1m, §2.3m) stay where they are** — they are the measured *evidence*, and folding never means deleting the measurement that bought the row. 🆕 **v22 (2026-09-20) CLOSES Z4: `.wat` pin digests are PORTABLE** — 954 of 954 comparable files agree, up from 2. ⚠️ **The v17 annex named ONE cause and there were THREE**; the other two (an over-eager data-count section, and implicit import types interned after every tag's) were reachable only by the gate, because the file the byte-for-byte diagnosis ran on could not contain either. 🚨 **v22 also carries a NEW wazmrt defect the run found — B-d: `(type N)` naming an undeclared type binds an import to a signature the text never wrote, rc 0**, where wasmrt refuses. 🆕 **v23 (same day) FIXES B-d, and cross-runtime parity is now TOTAL: the two pin listings are 1,467 lines each, `.wat` 954 of 954 and `.wasm` 513 of 513, with nothing in one and not the other.** 🆕 **v24 lands §2.1's four SUBCOMMAND SPELLINGS — `run`/`wasi`/`wat`/`wast`, additively — which EMPTIES the 2026-09-19 outstanding list; `pin` on wasmrt's side is what is left in §2.1.** ⏳ **wasmrt's copy is therefore at 10 and BEHIND — PENDING MIRROR**, which §1a says the owner directs and wasmrt makes in its own tree · opened 2026-08-19 (owner) · 🆕 **v25 records two wazmrt defects that broke BINARY interop in both directions** — a block type emitted as an internal enum tag, and the `any`/`extern` conversion opcodes swapped at four agreeing sites — found by assembling the SPEC corpus with both toolchains; the real-world `.wat` parity claim is unchanged at 954/954, which is exactly why that corpus could not find them. · last change 2026-09-20 (v25) · **this file must be kept IDENTICAL
...
🔒 **THE RETIRED ORACLE'S SCOPE, SETTLED BY THE OWNER 2026-09-20 — it covers `wasmrt` AND NOTHING
ELSE.** `reference-projects.md` had a *"What to mine it for"* column and the sentence *"we study them
freely"*; this file said *"reading a competitor's implementation for guidance remains off-limits"*.
Both were current and binding-sounding and they could not both be followed. **The narrow reading is
the one that governs:** §1's subject is the sibling, so

| | reading their source for DESIGN GUIDANCE |
| --- | --- |
| **`wasmrt`** (the sibling) | 🚫 **off-limits by default** — each time needs an owner-directed scoped exception, logged. Two have been granted: Track B-e, and the `@custom` review |
| **the other eight** in `reference-projects.md` | ✅ **allowed** — that file's "mine it for" column stands, and adoption stays gated by the Adoption Checklist + a Component Ledger entry in `third_party/LICENSES.md` |

⚠️ **What is NOT restricted for either, and never was: behavioural MEASUREMENT.** Running another
implementation and comparing its output, bytes or exit codes is the method this project relies on most
— the whole custom-annotations design was measured off wasm-tools that way. *Measuring what a binary
DOES is not reading how it does it.*
...
| call an exported function | `wazmrt <module> <export> [args…]` **and `run <module> <export> …`** | `wasmrt run <file> <fn> [args…]` **and the bare form** | ✅ **AGREED — both spellings on both sides.** wasmrt's F1 closed 2026-09-19; wazmrt took `run` 2026-09-20 (v24). 🔒 `run` does not fall back to WASI |
| run a WASI `_start` command | `wazmrt <module> [flags] [-- argv]` **and `wasi [flags] <module> …`** | `wasmrt wasi [flags] <file> […]` **and the bare form** | ✅ **AGREED 2026-09-20 (v24).** ⚠️ Under `wasi` the flags may precede the module on BOTH runtimes; `wasi` does not fall back to a summary |
| run a `.wast` spec script | `wazmrt <script.wast>` **and `wast <file\|dir>… [-v]`** | `wasmrt wast <file\|dir>… [-v]` **and the bare form** | ✅ **AGREED 2026-09-20 (v24).** 🔒 wazmrt gates every script, multi-target included — the path executes |
| assemble `.wat` → `.wasm` | `wazmrt wat <file.wat> [-o out]` | `wasmrt wat <file.wat> [-o out]` | ✅ **AGREED — wazmrt grew it 2026-09-20 (v24).** Without `-o` both assemble and report the size, writing nothing |
...
| # | item | wazmrt today |
| --- | --- | --- |
| **the subcommand spellings** | `run`, `wasi`, `wast`, `wat` | ✅ **CLOSED 2026-09-20, v24** (`8e04263b`) — all four accepted, additively, every bare form unchanged. The cell is the measurement that bought the row |
...
| **Z1 / Z2 / Z3** | §2.3, §2.4a, §2.5 | ✅ **CLOSED 2026-09-19, v21** (`0a48957a`) — the cells above are the measurement that bought the rows and stay as written |
| **Z4** | the `name` section | ✅ **CLOSED 2026-09-20, v22** (`a495424f`) — **954 of 954 comparable `.wat` digests agree**, up from 2. See §3.1m |

✅ **ALL SIX ROWS MEASURED ON 2026-09-19 ARE NOW CLOSED.** The `::` and feature-vocabulary rows and
Z1–Z3 closed with v21; Z4 with v22; B-d with v23; the subcommand spellings with **v24**. ⚠️ Closed
means *measured closed*, not *finished forever* — §2.1's table is the live one, and the next
`coordinate` re-runs it rather than trusting this line.
...
| --- | --- | --- | --- |
| `--dir <host>[<sep>guest]` | ✅ **BOTH `:` and `::`** (2026-09-19, v21) — a drive letter is exactly one ASCII letter at the start | ✅ **BOTH `:` and `::`** (2026-09-19), never mistaking a drive letter | ✅ **AGREED — the break is CLOSED.** ⚠️ One case is documented rather than guessed away and it is the same on both: `x:/` means drive `X:`, never the directory `x`; write `./x:/` or `x::/` |
| `--ro-dir` | same separator issue | same | ⚠️⚠️ as above |
| **flag POSITION** relative to the module path | flags come **AFTER** the path — 🆕🔒 **and a wazmrt flag written where only the guest sees it is an ERROR, `rc 1`** (v26, owner 2026-09-20; it WARNED from H7, 2026-08-19 until then); nothing after an explicit `--` is examined, which is now the whole escape hatch. ⚠️ **See §5 #12** — this contradicts §2.4a's *"guest positions are never examined"* and `prog.wasm install --yes` now fails | flags come **BEFORE** the path | ⚠⚠ **DIVERGENT AND LIVE — MEASURED 2026-08-19 (F3).** wasmrt silently passes a trailing `--dir` to the GUEST, so the sandbox is never granted and nothing warns. |
...
| `--` ends host flags, rest is guest argv | ✅ — 🆕 **and it is now the ONLY way to hand a wazmrt-flag spelling to the guest** (v26; see the position row below) | ❌ (preopens must precede the path) | ⚠️⚠️ **wasmrt must add, and this got MORE urgent with v26**: without `--`, a wasmrt user has no spelling for a guest argument that collides with a host flag name |
...
| a `.wat` input hashes the **assembled** bytes, not the source text | ✅ **AGREED, and ✅ PORTABLE since 2026-09-20 (Z4 closed, v22).** 954 of 954 comparable `.wat` files in the `wasmtk` tree now hash identically under both runtimes, up from 2; `.wasm` was and stays 513/513. **See §3.1m for the 2026-09-19 divergence and §3.1m-r for the close.** ✅ **And since v23 the two listings are the same length — 1,467 lines each, nothing in one and not the other** (B-d fixed, §3.1m-r). |
...
##### ✅ 3.1m-r — Z4 IS CLOSED · **CONTRACT VERSION 22** (wazmrt, 2026-09-20, `a495424f`)

**Re-measured with the same command on the same tree, the sibling rebuilt from its committed
`ab13ba975` into a scratch target dir (nothing written to its tree, §1a):**

| | `.wat` agree | differ | `.wasm` agree |
| --- | --- | --- | --- |
| 2026-09-19 | 2 | 952 | 513 |
| **2026-09-20** | **954** | **0** | 513 |

🔑 **The annex above named ONE cause and there were THREE. The other two could only be found by the
gate, not by the diagnosis** — and the diagnosis was not wrong, it was complete for the file it was
run on. `mathlib.wat` has no unused data segment and no tag, so neither of the other two causes could
show up in it. *A single-file byte-for-byte confirmation proves the cause it finds and says nothing
about the ones it cannot reach.*

| # | cause | fixed by |
| --- | --- | --- |
| 1 | no `name` section | emit it, twelve subsections, shape read off a wasm-tools-assembled module |
| 2 | data-count section emitted for **every** module with a data section | emit it exactly when a body names a segment, per §5.5.16 — which is what wasm-tools does |
| 3 | an imported function's implicit type interned **after** every tag's | intern at the import, in source order alongside the tags; defined functions stay a later pass |

⚠️ **The operator consequence in §3.1m is now the other way round and the docs must follow: `.wat`
pins ARE portable between the two runtimes** for every file both assemble. ⏳ **PENDING MIRROR** —
wasmrt's copy is behind; the owner directs that edit (§1a).

🚨 **AND A NEW wazmrt DEFECT THE RUN FOUND, which is why the count is 954 and not 956.** Two corpus
files (`bindgen_fixtures/fnany_50.wat`, `hostfn_50.wat`) write `(type 0)` / `(type 1)` in an import's
typeuse while declaring no types at all. **wasmrt refuses them** (*"inline function type doesn't match
type reference"*). **wazmrt assembles, validates and runs them — with the imports bound to signatures
the text never wrote**: `__host_print`, written `(param i32 i32)`, comes out as
`(param i32 i32 i32 i32) (result i32)`. `rc 0`, no warning. 🔑 The cause is time-of-check:
`checkInlineTypeUse` skips itself when the referenced index is not interned *yet*, deferring to a
"downstream verdict" that never arrives because the emitter later manufactures a type at that index.
**This is a §2.5-class failure (a truthful validity claim) and it is wazmrt's.** Tracked as **B-d**.

✅ **B-d IS FIXED — CONTRACT VERSION 23, same day** (`973dc0de`). Every typeuse carrying both forms is
recorded and answered once the type space is complete; both files are refused here as wasmrt refuses
them. 📊 **The two listings are now identical in length, 1,467 lines each — `.wat` 954/954, `.wasm`
513/513, nothing in one and not the other.** ⚠️ The fix is *ask later*, not *refuse a forward
reference*: a forward `(type N)` whose inline signature agrees with what lands at N is legal and
wasm-tools assembles it. 🔒 The out-of-range hand-off is kept — `(func (type 5))` with no types
assembles on both and is refused by both at decode.
...
| ~~2~~ | ~~**Who accepts whose CLI spelling, and by when** (§2.1)~~ | ✅ **CONFIRMED by the owner 2026-09-20 — and wasmrt is SYNCING ITS CLI TO wazmrt'S.** wazmrt's half landed in B-c4 (`run`/`wasi`/`wat`/`wast`, both `--dir` separators); the sibling adopting the rest closes the additive plan. ⚠️ **The ⬜ rows in `roadmap.md`'s §2.1 tables were stale when this was decided** — they still read "not started" for four capabilities that shipped; corrected the same day |
...
| ~~6~~ | ~~**Attribution of commit `7ce0dcd2` in the wasmrt repo**~~ | ✅ **CONFIRMED by the owner 2026-09-20** — the record stands as written and the matter is closed on wazmrt's side. 🔒 Nothing for an agent to do here and nothing to reopen: **wazmrt does not write to that tree**, and the owner carries anything that needs carrying. *(Original entry: )* wasmrt reports it committed **wazmrt's** §3.7a rewrite into its tree as if it were its own, before the collision was noticed. ⚠️ **wazmrt cannot fix this — §1a forbids writing to that tree**, and rewriting another repo's history is not an agent's call anyway. It is exactly the tracking-integrity problem the boundary was added to prevent, and it is now **behind** the rule rather than in front of it. Options: leave it with the collision documented in row 3b, or have the wasmrt session amend/annotate it **in its own tree** |
| ~~7~~ | ~~**Whether coordination should run in ONE session at a time** (§1c)~~ | ✅ **DECIDED by the owner 2026-09-20: YES — coordination runs ONE SESSION AT A TIME.** 🔒 **And the owner restated the boundary alongside it: *"we do not modify the other project unless specifically requested."*** That is §1a unchanged and now doubly stated — one session removes the same-copy last-write-wins race, and the write ban removes the cross-tree one. ⚠️ **"Specifically requested" means the owner, for a named edit.** `coordinate` is still an order to CONFER, never a licence to write. *(Original entry: )* §1a removes the cross-tree risk by construction, but two sessions editing the *same* copy still resolve last-write-wins with nothing to detect it. wasmrt proposed the discipline; it costs nothing and closes the residual gap |
...
| ~~11~~ | ~~**Should H7's WARN become an ERROR?**~~ (§2.2 position row, §2.4a-i) | ✅ **DECIDED by the owner 2026-09-20: YES, IT ERRORS.** ✅ **wazmrt shipped it the same day** — `misplacedHostFlag` + `reportMisplacedHostFlag` replace `warnMisplacedFlags`; `rc 1`, the message names both correct spellings, **byte-neutral in ReleaseSmall** (all three ceilings unmoved). ⏳ **PENDING MIRROR — wasmrt's §2.2 row still says WARN.** 🎓 The code argued *"warn, do not refuse, because a guest may legitimately take `--dir`"* — **and was wrong about its own escape hatch**: that case already had an exact spelling, `tool.wasm build -- --dir src`, which the walk always honoured. *A rule protecting a case the grammar can already express protects nothing; it only defers the ambiguity to whoever reads the output.* 🚨 **BUT THE ROW'S OBJECTION WAS REAL AND IS NOW LIVE — see §2.4a-ii below.** *(Original entry: )* a *known* host flag in a *guest* position warns today, because it may genuinely be the guest's. v14 does not reach it — the host/guest split is **grammar, not resemblance** — but it is the rule's nearest neighbour. ⚠️ **Erroring would break `prog.wasm install --yes`**, which §2.4 says must reach the guest untouched, so this is a real trade and not a tightening |
| 12 | 🆕 **§2.4a-ii — H7-as-error CONTRADICTS §2.4a's "guest positions are never examined". Which text wins?** | 🔬 **MEASURED after shipping #11, not predicted:** `wazmrt prog.wasm install --yes` now **errors, rc 1**. §2.4a says a guest's argv *"is not addressed to wazmrt at all"* and is never examined; §2.4 records the `install --yes` trap as the reason. ⚠️ **Both cannot stand as written.** 🎓 The honest framing: §2.4a's "never examined" **was already not literally true** — H7 warned about exactly these tokens since 2026-08-19. #11 changed the VERDICT, not the examination, and that is what made the contradiction visible. **Three ways out, for the owner:** **(a)** accept it — `install -- --yes` becomes the only spelling, and §2.4a is amended to say guest positions are examined for EXACT host-flag names; **(b)** restrict the error to the fail-OPEN flags (`--verify`, `--pins`, `--max-*`) and keep warning for the rest, which splits on the harm rather than on the grammar; **(c)** revert #11. 📌 **Recommendation: (a).** It is the only one that leaves a single rule, the escape hatch is exact, and (b) reintroduces a list that drifts — the failure mode v14 exists to prevent |
...
| version | date | change |
| --- | --- | --- |
| **26** ⏳ | 2026-09-20 | 🔒 **FOUR OWNER DECISIONS, AND THE CLI QUESTION IS CLOSED FROM BOTH ENDS.** **§5 #2 CONFIRMED — and wasmrt is SYNCING ITS CLI TO wazmrt's**, which finishes the additive plan (wazmrt's half landed in B-c4). **§5 #6 CONFIRMED** — the `7ce0dcd2` attribution record stands; nothing for an agent to do, since wazmrt does not write to that tree. **§5 #7 DECIDED: coordination runs ONE SESSION AT A TIME**, with the boundary restated by the owner — *"we do not modify the other project unless specifically requested"*, §1a unchanged and now doubly stated. **§5 #11 DECIDED: H7 ERRORS** — ✅ wazmrt shipped it the same day (`misplacedHostFlag`, `rc 1`, byte-neutral in ReleaseSmall). 🚨 **AND SHIPPING IT OPENED §5 #12, MEASURED RATHER THAN PREDICTED:** `wazmrt prog.wasm install --yes` now errors, which **contradicts §2.4a's "guest positions are never examined"** and the `install --yes` case §2.4 records. The old #11 row called that trade and was right. 🎓 §2.4a's "never examined" was **already not literally true** — H7 warned about these same tokens from 2026-08-19; #11 changed the verdict, not the examination, and that is what made the contradiction visible. Three ways out are written into §5 #12 with a recommendation. ⚠️ **§5 #10 IS STILL OPEN** — whether v14's "no looks like" reaches flag VALUES, i.e. `--dir`'s drive-letter fallback. It was not part of this round. 🔒 **Separately, and not a contract row: the RETIRED ORACLE's scope is settled** — it covers the sibling `wasmrt` only; the eight other reference projects may be studied, and behavioural measurement of anything was never restricted. |
| **25** ⏳ | 2026-09-20 | 🚨 **TWO wazmrt DEFECTS THAT BROKE BINARY INTEROP IN BOTH DIRECTIONS, found by assembling the SPEC corpus with both toolchains and comparing bytes** (`d47ecd18`, Track B-a). **(1)** A block type that is a **non-null abstract ref** was emitted as wazmrt's INTERNAL enum tag — `(ref any)` is `0x64 0x6E` and wazmrt wrote `0x66`, which is not a wire encoding of anything. Our decoder read it back happily; **the sibling refused those bytes outright** (*"decode failed: unsupported instruction opcode"*), so wazmrt was producing modules no other runtime could load. **(2)** `any.convert_extern` and `extern.convert_any` had their **wire bytes swapped** in four places that all agreed — encoder table, decoder, doc comments, and `validate.zig`'s own switch on the raw byte. ⚠️ **That one was wrong in BOTH directions**: modules wazmrt assembled were refused by every other runtime, and correctly-encoded modules from any other toolchain were refused **here**. Fail-closed each way, which is why nothing noticed. 📌 **Neither is a contract ROW** — §0 keeps internal design out of scope and neither is a CLI or security surface — **but both are recorded, because they are exactly the class §3.1 exists to protect: bytes that must mean the same thing to both runtimes.** 📊 Spec-corpus byte agreement went 1,807 → **2,121 of 2,141**; the residual 20 are two legal shorthand choices, cross-decoded on both runtimes with both encodings and documented at `roadmap.md` → B-a. ✅ **The real-world `.wat` parity claim is UNCHANGED at 954/954** — none of these constructs occurs there, which is precisely why that corpus could not find them. 🛠️ The check is committed as `tools/emitter-diff.mjs`, optional and outward-reaching like the Bake Off. |
| **24** ⏳ | 2026-09-20 | ✅ **§2.1 — THE SUBCOMMAND SPELLINGS ARE IN, AND THE 2026-09-19 LIST IS NOW EMPTY** (`8e04263b`, Track B-c4). `wazmrt run <module> <export> [args…]`, `wasi [flags] <module> [-- argv]`, `wat <file.wat> [-o out]`, `wast <file\|dir>… [-v]` — all four **additive**, with every bare-path form verified unchanged, so a command line written for either runtime now runs on both. 🔑 **`run` and `wasi` are aliases onto the EXISTING paths**, not copies: they set a mode and move the argument index, and everything after is parsed by the same code, so the verify gate, `--features`, `--max-iterations` and Z1/Z2/Z3 apply identically whichever spelling was used. 🔒 **The named modes do NOT fall back** — `run m.wasm nosuch` on a module that also exports `_start` would otherwise run `_start` with `nosuch` as guest argv and exit 0, which is §2.3's failure in a new position; the bare form's guard stays narrow because there the word really might be argv. 🆕 **`wat` closes §2.1's *"wazmrt must grow it"* row**; measured from the sibling rather than assumed, it writes **nothing** without `-o` and only reports the size. ⚠️⚠️ **The one real trap, recorded because it is fail-OPEN:** `wasi`'s leading flags are moved behind the path as `lead ++ tail` — `tail ++ lead` would place them after an explicit `--`, handing `--dir` to the guest as argv so the run proceeds **with no preopen and no message**. It is a named function with its own inversion-proven test, and confirmed end to end. 🧹 Also fixed: the unknown-flag hint advised *"use `--` to pass it to the guest"* from `.wast`, summarize, `wat` and `run`, none of which have guest argv — a §2.5-class untruth in a diagnostic, now per-mode. 📏 exe +9,216, **lib +0, dll +0** — none of this is in `root.zig`, so CLI convergence costs the embed artifacts nothing. ⏳ **wasmrt's copy is still at 10 — PENDING MIRROR**, and §2.1's `pin` row remains wasmrt's half. |
| **23** ⏳ | 2026-09-20 | ✅ **B-d IS FIXED and the two pin listings are now IDENTICAL IN LENGTH — 1,467 lines each** (`973dc0de`). `(type N)` naming a type the module has not declared yet is checked against the **finished** type space instead of an empty one, so `bindgen_fixtures/fnany_50.wat` and `hostfn_50.wat` are refused here exactly as wasmrt refuses them, with a dedicated `InlineTypeUseMismatch` rather than a generic *"bad module field"*. 📊 **Cross-runtime parity is therefore total: `.wat` 954 of 954, `.wasm` 513 of 513, nothing in one listing and not the other.** 🔑 The cause was time-of-check, not a missing check: the text format's type index space is the declared `(type …)` defs *plus* every implicitly interned signature, appended as they are met, so a check run where the typeuse is parsed can be looking at an empty space — it skipped itself and deferred to a downstream verdict the emitter then prevented from ever arriving. ⚠️ **The fix is "ask later", NOT "refuse a forward reference"**: a forward `(type N)` whose inline signature *agrees* with what lands at N is legal and wasm-tools assembles it, so that case is pinned by its own assertion — a blanket refusal would have passed every failing case and broken the legal one. 🔒 **The out-of-range hand-off is kept and is now truthful** — wasm-tools assembles `(func (type 5))` in a module with no types and **both** runtimes refuse those bytes at decode, verified by running both. ⬜ **Still open on wazmrt: the four subcommand spellings (§2.1)** — Track B-c4, and it is the last row of the 2026-09-19 measurement. |
| **22** ⏳ | 2026-09-20 | ✅ **Z4 IS CLOSED — `.wat` pin digests are PORTABLE** (`a495424f`, §3.1m-r). Re-measured with the same command on the same tree, the sibling rebuilt from its committed `ab13ba975` into a scratch target: **954 of 954 comparable `.wat` files agree, up from 2; 0 differ.** 🔑 **The v17 annex named ONE cause and there were THREE**, and the other two were reachable only by the gate — `mathlib.wat`, the file the byte-for-byte diagnosis was run on, has neither an unused data segment nor a tag, so neither could appear in it. 🎓 *A single-file confirmation proves the cause it finds and says nothing about the ones it cannot reach.* **(1)** the `name` section was never emitted — twelve subsections now are, with the shape **read off a wasm-tools-assembled module** rather than recalled, and label entries keyed by the block's **ordinal in the body** (counting unnamed blocks), never by stack depth; **(2)** the data-count section was emitted for **every** module with a data section — legal, and 3 bytes no other assembler writes — now emitted exactly when a body names a segment, per §5.5.16; **(3)** an imported function's implicit type was interned **after** every tag's, because tags intern during the field loop and imports interned in a pass after it — now interned at the import, in source order alongside the tags, with defined functions still a later pass, which is what wasm-tools does. ⚠️ **§3.1's operator note inverts: `.wat` pins no longer have to be made per runtime.** 🚨 **AND A NEW wazmrt DEFECT THE RUN FOUND (B-d), which is why the count is 954 and not 956:** `(type N)` naming an undeclared type in an import's typeuse is refused by wasmrt and **silently mis-bound by wazmrt** — `__host_print`, written `(param i32 i32)`, is emitted as `(param i32 i32 i32 i32) (result i32)`, rc 0. `checkInlineTypeUse` skips itself when the index is not interned *yet* and defers to a downstream verdict the emitter then prevents. **A §2.5-class failure and wazmrt's to fix.** ⬜ **Still open on wazmrt: the four subcommand spellings (§2.1)** — Track B-c4. |
| **21** ⏳ | 2026-09-19 | ✅ **wazmrt ADOPTS: Z1, Z2, Z3, the `--dir` fix, the pin DB order + warning, and wasm-tools' feature vocabulary** (`0a48957a`, `22437455`). Z1 fails with rc 1 naming the export; Z2 says `unknown flag` in every host position with no path-guessing, and leaves guest positions alone; Z3's header is neutral. §2.2's `--dir` is fixed per the owner's §5 #10 ruling — a drive letter is **exactly one ASCII letter at the start**, so `.:/` splits and `C:\tmp` does not, and `::` is accepted. §3.3's lookup is shared-path → own-path → **warn when the sibling's DB exists and ours does not**, as a pure function with all eight combinations tested. §2.2's `--features` resolves wasm-tools' whole vocabulary. 🚨 **AND A SECURITY DEFECT FOUND WHILE DOING IT, recorded because it is a contract-surface failure: `--allow-symlink` was missing from the shared flag list, so `wazmrt start.wasm --allow-symlink --verify enforce` RAN AN UNVERIFIED MODULE, rc 0**, where the same line without it correctly refuses. The flag region ended at it and every later host flag went unseen — `--max-iterations` dropped too. 🎓 *The "one list, because a second copy would drift" comment sat directly above that list; the list was in one place and the parser was reading a different vocabulary.* ⬜ **Still open on wazmrt: Z4 (§3.1m) and the subcommand spellings (§2.1)** — Track B-c2 and B-c4. |
