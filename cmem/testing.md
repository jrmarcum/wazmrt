...
## Reading the test count (updated 2026-07-19, post code-audit + strictness fixes)

`zig build test --summary all` prints **360** (356 pass, 4 skip), but there are **185 distinct tests**:
175 in the core module (173 pass + 2 skip) + 10 C-ABI. The `cabi_tests` target's root is
`wasm_c_api.zig`, which imports `root.zig`, so it compiles and **re-runs the core module's tests too**
(175 core + 10 C-ABI = 185), on top of the standalone `mod_tests` run (175) → 360 printed. Harmless —
under a second — but **don't quote the printed number as a test count**; quote **173**, or the per-target
