...
| `opcode.zig` | The shared instruction authority: `Op` enum (core-MVP 0x00–0xC4 + `table.get`/`.set` 0x25/26 + reference types 0xD0–D2 + `0xFC` table ops via internal tags/`fcSubOpcode`), `Imm`/`Instr`, `immediateKind`, `decodeBody`. ⚠️ **Internal tags share the single-byte space with real opcodes, and `Op` has ONE unassigned byte left** — D3 took `0x1d`/`0x1e`/`0x27` for custom-descriptors. `decodeBody`'s raw-byte guard exists solely because of that sharing, and it has been wrong three times (R3, R10, and a fourth arm D3 added then removed as redundant). **Widen `Op` to `enum(u16)` and move every internal tag above `0xff` before adding more** — after which no raw byte can name one and the guard collapses to nothing. |
