...
| [known-issues.md](known-issues.md) | **Deferred audit findings (to-fix-later) + the integration that surfaces each** (2026-07-09 audit). Grep this before starting `register`/linking, imported funcs, `assert_invalid`, start-functions, or host externrefs. `assert_trap`-accepts-any-error (#5) is the one that hurts *now* (green-washes producer bugs) |
