...
| [known-issues.md](known-issues.md) | **Audit-finding ledger + the integration that surfaces each** (2026-07-09 audit + post-audit fixes). #1–#16 all **DONE** (cleared 2026-07-13). **Three open items from Phase 3 (2026-07-15), none blocking Phase 4:** **#17** the WASI sandbox is *lexical* — a symlink out of a preopen is still followed; **fix before wazmrt ever runs untrusted modules**, not before Phase 4. **#18** a real Zig 0.16 std bug (`openFile(.follow_symlinks=false)` on Windows returns an async handle marked non-blocking → `.PENDING => unreachable` **crashes the host**); worked around in `wPathOpen`, **recheck on every Zig upgrade**. **#19** traps carry no location (`trap: Unreachable`, no fn/pc/name) — the thing that made #18/the `bitcast_invalid` hunt cost hours; **highest-leverage item to do first in Phase 4**, since Phase 4 is "run many unfamiliar compiled guests." Out-of-scope: multi-memory, EH tags. Records which fixes shipped in which commit |
