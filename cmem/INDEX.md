...
| [known-issues.md](known-issues.md) | **Audit-finding ledger + the integration that surfaces each** (2026-07-09 audit + post-audit fixes). **Effectively cleared 2026-07-13** — #1–#7, #9, #10, #12–#16 DONE; only #8 (non-power-of-two `align=`) and #11 (defined-table inline export) remain, both LOW/fail-loud/untested. Next wins = invoke-by-module-name, then new features (C ABI, WASI). Out-of-scope: multi-memory, EH tags. Records which fixes shipped in which commit |
