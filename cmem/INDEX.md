...
| [testing.md](testing.md) | Unit tests (**73**) + the external conformance corpora (`wasm_mod`, `wasm_wast/testsuite-main`, `wasm_wasi`). **Spec-testsuite snapshot** — positive *and* negative conformance run: i32 459/0, call_indirect 169/0, func 171/0, **table_init 729/0, table_copy 1649/0**, elem 63, data 34/0, **imports 137, linking 100/37, start 11/0**, and the **function-references** files (`ref_null` 32/0, `ref_is_null` 18/0, `call_ref` 30/1, `return_call_ref` 38/9, `ref_as_non_null` 5/0, `local_init` 8/0), global 109/1 (f32/f64 2498/2). **2026-07-13:** #15 + host imports #1 + start (#3) + audit-ledger closeout + invoke-by-module-name + **function-references (P1/P2/P2.5)** |
