...
| [testing.md](testing.md) | Unit tests (**63**) + the external conformance corpora (`wasm_mod`, `wasm_wast/testsuite-main`, `wasm_wasi`). **Spec-testsuite snapshot** — positive *and* negative conformance now run: i32 459/0, i64 415/0, block 222/0, if 240/0, call_indirect 169/0, select 154/0, func 171/0, align 140/0, custom 8/0, binary-leb128 58/1, global 108/2 (f32/f64 2498/2). Malformed over-acceptance ~zero; remaining gaps are host imports + init-expressions, not core bugs |
| [text-toolchain.md](text-toolchain.md) | **Native WAT assembler + WAST runner** for the standard `.wast` format. `sexpr.zig` + `wat.zig` + `wast.zig` all DONE — **runs the official spec testsuite** (positive + `assert_invalid`/`assert_malformed`/`assert_exhaustion`; `assert_trap` gated on a real trap). Assembler covers multi-value/type-index block types, `call_indirect` + multi-table + `elem`, globals (imported + extended-const), reference types, reference-type table ops. Next: `register`/linking + imported functions, then init expressions |
| [known-issues.md](known-issues.md) | **Audit-finding ledger + the integration that surfaces each** (2026-07-09 audit + post-audit fixes). Most tractable items DONE (#2/#5/#6/#7/#8/#14/#16); grep before starting `register`/linking, imported funcs, start-functions, host externrefs, or table/element init exprs (#15). Records which fixes shipped in which commit |
| [roadmap.md](roadmap.md) | Current status + next increments. **Done:** decode, validate, execute (int/float/memory, runs the `wasm_mod` corpus + spec testsuite incl. negative conformance), full text toolchain, **reference types + multi-table + imported globals + extended-const + reference-type table ops + validator/decoder strictness (2026-07-09)**. **Next:** `register`/module-linking + imported functions (→ host imports / WASI), init expressions, grow the C ABI, the Deno/V8 benchmark, loader integration |
