...
| [testing.md](testing.md) | Unit tests (**65**) + the external conformance corpora (`wasm_mod`, `wasm_wast/testsuite-main`, `wasm_wasi`). **Spec-testsuite snapshot** — positive *and* negative conformance run: i32 459/0, if 240/0, call_indirect 169/0, select 154/0, func 171/0, align 140/0, custom 8/0, binary-leb128 58/1, **table_init 729/0, table_copy 1649/0**, elem 47, data 31, func_ptrs 32/0, global 109/1 (f32/f64 2498/2). **#15 fully closed 2026-07-13** (bulk table ops + table-init exprs + const-expr data offsets); the only remaining `data`/`elem` gap is imported tables/memories (#1 stage 2) |
