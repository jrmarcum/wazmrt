...
├── build.zig.zon          # Package manifest (name .wazmrt, v1.0.1, min zig 0.16.0)
...
- `zig build test-shipped` → 🆕 **Track H, 2026-08-19** — the same suite under **`ReleaseSmall`, the config that actually ships**, where Zig's safety checks are OFF. Distinct from `test-safe` (ReleaseSafe, checks KEPT): that one asks *does any input reach a bad cast or index*, this one asks *does the SHIPPED build behave the same* — a miscompile or optimizer-exposed UB appears here and nowhere else. ⚠️ It was runnable by hand all along (`-Doptimize=ReleaseSmall`) and nobody ever ran it; **a flag you must remember is not a gate**.
- `zig build test` → runs the unit tests (**798/798 as of 2026-09-20 (Track B)** — was 765 at 2026-08-19 (Track H) — was 758 after 2026-08-18 (Tracks F, L and A, the skip-closing pass, wide-arithmetic)** — THREE test binaries now: `main.zig` gained `cli_tests`, its first ever, because `--features` put a policy parser in the front end; *superseded: 715/715 at 2026-08-17 end of day, and 709, 691, 667, 651 and 633 earlier that day, 603 at 2026-08-13* — 510 at 2026-08-11, then R1/R2 regression tests, then R3's 7, R4's 4, R5's 3 and R10's 5 (×2 targets: the C-ABI target re-runs the core tests, so each new core test raises the printed total by two); ZERO skips from an NTFS cwd, 4 skipped from this repo’s own `D:` cwd because exFAT has no symlinks; green under Debug AND ReleaseSafe; see `testing.md`)
...
- `zig build size -Doptimize=ReleaseSmall` → fails the build if a shipped artifact grew past `tools/size-ceilings.txt`. **Live 2026-09-20 (Track B): exe 1,009,152 / static lib 1,059,236 / dll 910,336** — all three BUILT AND verified in one run, gate EXACT. 🆕 A `-Doptimize=ReleaseSmall` build now runs this gate **by itself** (B-e). ⚠️ Every `dll` figure recorded before 2026-09-20 was read off a stale artifact — the gate did not build the shared library it graded. 🔑 **The DLL moved by ZERO through BOTH passes.** Track F`s +5,632 exe is entirely the CLI `--features` surface (no embedder carries `main.zig`); the skip-closing pass added +2,560 exe / +1,966 lib, the shape decoder+assembler work has. A flat DLL across both is the evidence neither leaked into the embed artifact. *(Superseded: 978,944 / 1,046,522 / 900,608 at 2026-08-17 end of day.)* ⚠️ **Measure in the REFERENCE CONFIGURATION** (a `C:` copy of the tree with its own default `.zig-cache`): the static lib's byte size varies with the source and cache PATHS, so the same commit reads 1,029,730–1,029,922 depending on where it was built — see the gate-hole note in `tools/size-ceilings.txt` before charging a sub-KB delta to a change. *(Superseded: 964,608 / 1,029,730 / 888,832 earlier the same day; 2026-08-13 after R10 — exe 953,344 / static lib 1,016,766 / dll 878,592.)* ⚠️ Nothing invokes this automatically and the ceilings drifted 22–25 KB unnoticed between 2026-08-11 and 2026-08-13 — **run it before any commit touching `src/`**, and measure the static lib from a PLAIN `zig build` (`zig build dll` overwrites `wazmrt.lib` with the DLL import library). ⚠️ **`zig build` does not build the DLL at all**, so the gate happily reports whatever `.dll` is left in `zig-out` from a previous session — R3's first run showed the DLL at *exactly* its ceiling, which was a stale file, not a free change. **A delta of exactly zero is a reason to check the timestamp**; run `zig build dll -Doptimize=ReleaseSmall` before reporting a DLL number
...
- **Decode + validate + execute all work.** The pipeline decodes all core sections → the `opcode.zig`
  ...
  **custom-descriptors** (Track D — `(exact $t)`, `(descriptor $d)`/`(describes $s)`,
  ...
  2026-08-17, and **wide-arithmetic** (`i64.add128`/`sub128`/`mul_wide_s`/`mul_wide_u`) on
  2026-08-18 — the cheapest proposal on record, ~3 KB for the largest single block of remaining
  skips. 🏁 **The runtime runs the official spec testsuite at ZERO FAILURES** — 284 files,
  **63,934 assertions passing / 0 failed / ZERO skipped** — plus the threads `atomic.wast` suite
  ...
  **What genuinely remains: TWO STANDING DELTAS, both holding for upstream revisions** (see `known-issues.md` — ⚠️ **SD-3 was RETIRED 2026-08-18**: pricing it prompted a re-test of its reopen condition, the condition had been met all along, and `delegate` shipped as Track L), and nothing else
  in the corpus. Track F closed 2026-08-18, the 144 skips it left are down to **ZERO**, and the last
  unrun file (`annotations.wast`) now runs — **the conformance baseline is EMPTY.**
  🔑 **Quote the FOURTH number too — "0 file(s) with failures/errors".** A file that fails to PARSE
  is invisible in passed/failed/skipped, which is how `annotations.wast` stayed unpriced behind an
  otherwise perfect score.
  ⚠️ *(This line has named, in turn, "Track F's remainder", then "25 skips", then "one file that
  does not RUN". All three are done — and that succession is itself the lesson about how a
  remaining-work line ages.)*
