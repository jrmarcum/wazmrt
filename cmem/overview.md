...
- **Decode + validate + execute all work.** The pipeline decodes all core sections → the `opcode.zig`
  ...
  **threads/atomics** (the whole `0xFE` family + `shared` memories), and **GC constant expressions** are
  all done. **Phases 1–8 complete** — WASI, CLI ergonomics + conformance, pin verification, exception
  ...
  testsuite** (58.6k assertions passing) plus the threads `atomic.wast` suite (302/0), and assembles ~490
  of a real-world `.wat` corpus. **The only unimplemented wasm proposal left is memory64** (i64 addresses);
  otherwise what remains is one upstream Zig limitation — see `known-issues.md` and `roadmap.md`.
